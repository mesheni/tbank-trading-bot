...
### 3.6.2 Режим по размеру капитала (профиль: micro / small / standard)

Бот подстраивается под размер счёта: при старте он определяет режим по фактическому
капиталу (`CAPITAL_PROFILE=auto`) — долю позиции, лимит экспозиции и вселенную тикеров
(бумаги, чей лот дороже бюджета позиции, исключаются). Чтобы режим работы на сервере
соответствовал реальному размеру счёта, задайте бюджет в `.env` и переобучитесь под него:

```bash
cd /root/tbank-trading-bot
systemctl stop tbank-bot
# пример для счёта 30 тыс. руб (режим micro определится сам, но бюджет лучше явный):
grep -q '^ACCOUNT_BUDGET_RUB=' .env || echo 'ACCOUNT_BUDGET_RUB=30000' >> .env
.venv/bin/python cli.py train --capital 30000
.venv/bin/python cli.py backtest --capital 30000
systemctl start tbank-bot   # бот применит профиль по фактическому капиталу счёта
```

Профиль фиксируется на запуск процесса: смена режима (например, после пополнения счёта)
— это правка `.env` (`CAPITAL_PROFILE=...`, `ACCOUNT_BUDGET_RUB=...`) и рестарт юнита.
Порог kill-switch считается от `ACCOUNT_BUDGET_RUB`, поэтому без него на маленьком счёте
просадка от «миллиона по умолчанию» никогда не сработает.
